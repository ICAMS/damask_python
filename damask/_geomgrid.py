...
from typing import Optional, Union, Sequence, Tuple, List, Dict, Mapping, Any
...
class GeomGrid:
    ...
    def load_MiMedat(src: Union[str, Path, Mapping[str, Any]], keys: Dict[str, str] = None) -> 'GeomGrid':
        ...
        Expected JSON keys (customizable via `keys`):
          - "RVE_size": [Lx, Ly, Lz]               (axis lengths)
          - "discretization_unit_size": [dx,dy,dz] (voxel edge lengths)
          - "discretization_count": int            (Nx*Ny*Nz)  [optional; will be checked if present]
          - "origin": [ox,oy,oz]                   (optional, defaults to [0,0,0])
        ...
        if keys is None:
            keys = {
                "size": "RVE_size",
                "spacing": "discretization_unit_size",
                "count": "discretization_count",
                "origin": "origin",
                "units": "units",
                "length": "Length",
            }
        ...
        if isinstance(src, (str, Path)):
            src = str(src)
            try:
                # Treat as path if it points to an existing file
                p = Path(src)
                if p.exists() and p.is_file():
                    data = json.loads(p.read_text())
                else:
                    # Otherwise, try to parse as a raw JSON string
                    data = json.loads(src)
        ...
        if N_calc != N_json:
            raise ValueError(
                f"discretization_count mismatch: JSON={N_json}, computed={N_calc} from cells={cells.tolist()}")
        ...
        if isinstance(micro_list, list) and len(micro_list) > 0 and isinstance(micro_list[0], dict):
            ...
            if ("voxels" in micro0) and ("grains" in micro0):
                ...
                if u_vox.size != gids.size or not np.array_equal(np.sort(u_vox), np.sort(gids)):
                    raise ValueError(
                        "Mismatch between voxel 'grain_id' set and 'grains' list.\n"
                        f"voxels unique={u_vox.tolist()} vs grains={gids.tolist()}"
                    )

                # Build 0-based dense index for grain IDs using the order in `grains`
                gid_to_idx = {int(gid): i for i, gid in enumerate(gids)}  # 1->0, 2->1, ..., 21->20
                mapped = np.fromiter((gid_to_idx[int(g)] for g in grain_ids),
                                     # map voxel grain_id → 0..G-1
                                     dtype=np.int64, count=grain_ids.size)

                # Place into flat C-order vector, then reshape (C-order) to 3D
                labels_C = np.empty(N, dtype=np.int64)
